...
const DuLieuDaXoa = ({
  ...
  const expandable = {
    ...
    expandedRowRender: (record: any) => (
      <div
        ...
        onClick={() => handleSelectRow(record?.project_id)}
      >
        <p></p>
  ...
  const defaultColumns: any[number] & {
    ...
    },
    {
      ...
      dataIndex: 'selectAll',
      width: '10%',
      render: (_: any, record: { project_id: React.Key }) => (
        <Checkbox
          checked={selectedRowKeys.includes(record.project_id)}
          onChange={() => handleSelectRow(record.project_id)}
        />
      ),
      editable: true,
    },
