...
const EditCV: React.FC<any> = ({
  ...
  const handleOk = () => {
    type === '1'
      ? POST(
          `me/chi-tiet-cong-viec-cua-toi/project/${id}/edit-job-of-job/${newData?.id}`,
          newData
        ).then((res) => {
          if (res) {
      ...
      : POST(
          `me/chi-tiet-cong-viec-cua-toi/process/${id}/edit-mission-job/${newData?.id}`,
          newData
        ).then((res) => {
          if (res) {
            setIsModalOpen(false);
  ...
  const handleNameSubWork = (e: any) => {
    if (type === '1') {
      setNewData({ ...newData, job_name_job: e.target.value });
    } else {
      ...
      >
        <div>
          <div className={styles.name}>
            <p>Tên công việc</p>
            <input
              value={type === '1' ? newData?.job_name_job : newData?.job_name}
              type="text"
