...
import TodayMeeting from '@/components/quan-ly-cuoc-hop/Detail/TodayMeeting';
import Meeting_ST from '@/components/quan-ly-cuoc-hop/Detail/Meeting_ST';
import Meeting_DQ from '@/components/quan-ly-cuoc-hop/Detail/Meeting_DQ';
import Image from 'next/image';
import AddMeeting from '@/components/quan-ly-cuoc-hop/AddMeeting';
...
export default function PostPage({
  ...
  const onSearch = (value: string) => {
    POST_PARAM_QUERY('meetings/quan-ly-cuoc-hop', {}, { keywords: value }).then(
      (res) => {
        if (res) {
          setAllMeeting(res?.data?.TatCaCuocHop?.TatCaCuocHop);
          setIsComingMeeting(res?.data?.CuocHopSapDienRa?.CuocHopSapDienRa);
              ...
              <div>
                ...
                <div className="scrollmobile" id="navbar_meeting">
                  <div className="navbar">
                    <div
                      className={`all_metting ${
                        activeTab === 'all' ? 'active' : ''
                      }`}
                      onClick={() => handleTabClick('all')}
                    >
                      <label className="wind_alm windo_one">
                        <p className="numb_mett" style={{ margin: '0px' }}>
                          {count.all}
                        </p>
                        <p className="note_mett" style={{ margin: '0px' }}>
                          Tất cả cuộc họp
                        </p>
                        <input
                          type="radio"
